...
module.exports = deployer => {
  const args = process.argv.slice().filter((arg) => arg !== '--reset');
  if (process.env.DEPLOY_FACTORY){
    ...
    console.log("multi sig owners: "+args[3]);
    console.log("Required confirmations: "+ args[4]);
    deployer.deploy(MultisigWalletWithoutDailyLimit, args[3].split(",").filter(elm => elm !== ''), args[4]);
